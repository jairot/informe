...
Entre de dispositivos que realizan la clasificacion de paquetes se encuentran soluciones implementadas de manera completa en hardware, FPGA, ASIC u otra plataforma, o implementadas, procesador de proposito general mediante, completamente en software. La intención de de este proyecto integrador, como puede verse de manera grafica en la figura~\ref{fig:hwsw}, es complementar las ventajas de ambos mundos maximizandolas y reduciendo asi las desventajas de utilizar el hardware o el software por separado. 


 \begin{figure}[h]
  \centering
	 \includegraphics[width=0.7\textwidth]{2-sistema/graf/interseccion.eps}
...
Para implementar la solucion se propone el hardware descripto en el diagrama de la figura~\ref{fig:solucion}
 \begin{figure}[h]
  \centering
	 \includegraphics[width=0.8\textwidth]{2-sistema/graf/solucion.eps}
...
Considerar una dirección de destino \textit{D}. Para efectuar la búsqueda del prefijo más largo los bits de \textit{D} son usados para trazar un camino a lo largo del trie. Dicho camino comienza en el nodo raíz y continúa hasta toparse con un puntero vacío o un nodo vacío. Durante el recorrido a través del trie, el algoritmo mantiene un registro del último prefijo encontrado en un nodo en el camino. Cuando la búsqueda termina en un puntero vacío, ese es el prefijo retornado.
\newpage
\begin{figure}[h]
  \centering
...
A lo que es posible ver en la figura ~\ref{fig:diseno} es necesario agregarle algún tipo de interfaz de red que permita el ingreso de datos al sistema para luego procesarla. Además, según lo planteado en los objetivos de este trabajo, se desea implementar un sistema operativo o algún tipo de capa de abstracción que permita la ejecución de un Software en el que se realizará dicho procesamiento de estos paquetes. Para vincular estos dos módulos también es necesario un modulo que transporte los datos recibidos por la interfaz de red hacia el software y luego sobreescriba los resultados obtenidos en una etiqueta adjunta a los datos procesados. En la figura ~\ref{fig:diseno2} se puede ver como resulta el diseño propuesto.
